...
- A structured-data parse failure (malformed JSON, top-level JSON that isn't an object, an empty string, etc.) is treated the same as an existing "type conversion failure". A `key:` referencing a key that isn't present in the actual log line is treated the same way, regardless of the field's type (previously, a missing key silently became Go's zero value `""`, which a `string` field would happily accept; now a missing key is itself detected as an error) - unless the field also sets `optional: true` (see below).
- **`optional: true`** on a `key:` field stores Parquet `null` instead of failing when the structured data has no value for that key: the key is entirely absent, or (for any type other than `string`) present but empty (`""`, or an explicit JSON `null`, which decodes the same way). A `type: string` field with `optional: true` still stores an empty string as `""`, not `null` - only a genuinely absent key becomes `null` for strings. `optional:` is rejected on anything other than a `key:` field, and on `type: timestamp` (a null value there could silently drop a row picked as a multi-file merge key - see [Merge order across multiple input files](#merge-order-across-multiple-input-files)). Two rules sharing an output name must agree on `optional:` for every field, the same way they must already agree on name/type.
