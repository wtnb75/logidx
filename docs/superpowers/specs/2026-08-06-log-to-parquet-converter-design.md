...
古いsyslog形式(例: `Aug  6 12:34:56`)のように年を含まないタイムスタンプを、追加の設定項目なしで自動判別して補完する。

- `format`にGoの年トークン`2006`が含まれるかどうかで自動判別する。含まれていれば通常通りテキストから年を取得する
- 含まれていない場合、以下のロジックで年を確定する:
  1. プログラム起動時に1回だけ取得した基準時刻`now`の年を候補年とし、パースした月日時分秒と組み合わせて候補時刻を作る
  2. 候補時刻が`now`より未来なら、候補年を1つ前の年にする(例: 実行日が1月2日で対象行が「12月31日」なら前年と判定)
  3. これで確定した年を採用する
- `now`はCLI起動時に固定し、同一実行内では一貫した基準を使う

**既知の制約**: 1年以上前にローテーション済みの古いログファイルを今変換する場合、この推定は実際より新しい年を誤って割り当てる(年情報がない以上、原理的に回避不能)。
