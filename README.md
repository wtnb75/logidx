...
- 確定に失敗した(型変換エラーになった)複数行エントリは、先頭行だけが次の候補ルールで再試行され、2行目以降は1行ずつ独立に再処理される(前述の「変換失敗時は次の候補ルールにフォールバックする」を参照)。残る候補ルールすべてで失敗した場合、その先頭行だけが`unmatched.txt`に1レコードとして(本来の行番号で)書かれる。
